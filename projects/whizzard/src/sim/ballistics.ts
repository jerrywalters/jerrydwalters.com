...
const SUBSTEPS = 3; // sub-steps per frame for collision stability (prevents tunnelling)
...
export function integrate(pool: DropletPool, containers: Container[], hazards: Rect[], dt: number): IntegrateResult {
  ...
  for (let i = 0; i < drops.length; i++) {
    ...
    for (let sub = 0; sub < SUBSTEPS && d.alive; sub++) {
      const py0 = d.py;
      d.vy -= GRAVITY * subDt;
      d.px += d.vx * subDt; d.py += d.vy * subDt; d.pz += d.vz * subDt;
      ...
      for (let c = 0; c < containers.length; c++) {
        ...
        if (py0 > con.rimHeight && d.py <= con.rimHeight) {
          ...
          const cx = d.px - d.vx * subDt * (1 - t);
          const cz = d.pz - d.vz * subDt * (1 - t);
      ...
      if (d.py <= 0) {
        ...
        const fx = d.px - d.vx * subDt * (1 - t);
        const fz = d.pz - d.vz * subDt * (1 - t);
        let onHazard = false;
